...
SUPER_VERSION="5.0.0-beta3"
readonly SUPER_VERSION
SUPER_DATE="2024/09/09"
readonly SUPER_DATE
...
manage_parameter_options() {
	...
	# Initial validation for the ${display_icon_file_option}, ${display_icon_light_file_option}, and ${display_icon_dark_file_option} to set ${display_icon_light_file_path} and ${display_icon_dark_file_path}. Handaling of the actual display icon files themseves is in the manage_helpers() funciton.
	...
	if [[ "${mac_cpu_architecture}" == "arm64" ]]; then
		...
		if [[ "${scheduled_install_option_counter}" -gt 0 ]] && [[ "${auth_ask_user_to_save_password}" == "FALSE" ]] && [[ -z "${auth_local_account_option}" ]] && [[ -z "${auth_service_add_via_admin_account_option}" ]] && [[ -z "${auth_jamf_client_option}" ]] && [[ -z "${auth_jamf_account_option}" ]]; then
			log_super "Parameter Error: All --scheduled-install-* options require that you also specify a saved --auth-* option."
			option_error="TRUE"
		fi
...
# This function is only used for debuging from the command line to interrupt the workflow and wait for the user to press Enter to continue. Insert the following line wherver you want an interrupt to occour:
...
check_msu_settings() {
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_update_checks is: ${msu_automatic_update_checks}"
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_macos_download is: ${msu_automatic_macos_download}"
...
check_scheduled_install() {
		...
		if [[ -n "${schedule_workflow_active_option}" ]] && { [[ -n "${scheduled_install_days}" ]] || [[ $(echo "${scheduled_install_date}" | grep -c ':00:00$') -gt 0 ]]; }; then
			...
			if [[ -n "${workflow_scheduled_install_adjusted}" ]]; then
				...
				if [[ -n "${scheduled_install_days}" ]] && [[ $scheduled_install_days -eq 0 ]]; then
					log_super "Status: Setting new scheduled installation of $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} which is the start of the soonest workflow active time frame after zero day."
				else
					[[ "${schedule_workflow_active_matches_now}" == "TRUE" ]] && log_super "Status: Setting new scheduled installation of $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} which is the start of the current schedule workflow active time frame."
					[[ "${schedule_workflow_active_matches_now}" == "FALSE" ]] && log_super "Status: Setting new scheduled installation of $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} which is the start of the latest workflow active time frame on the original scheduled installation date."
				fi
			else
				[[ -n "${scheduled_install_days}" ]] && log_super "Warning: The current scheduled installation of $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} (${scheduled_install_days} days after zero day) is overriding the schedule workflow active option because no coordinating time frame could be resolved."
				[[ -n "${scheduled_install_date}" ]] && log_super "Warning: The current scheduled installation of $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} is overriding the schedule workflow active option because no coordinating time frame could be resolved."
			fi
		elif [[ -n "${schedule_workflow_active_option}" ]]; then
			...
			[[ -n "${scheduled_install_days}" ]] && log_super "Status: Setting new scheduled installation of ${workflow_scheduled_install} which is ${scheduled_install_days} days after zero date."
	...
	# If a scheduled install is active then set display strings and evaulate ${workflow_scheduled_install_now}.
...
check_software_update_status_cached() {
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_checksum_cache is: ${deferral_restrictions_checksum_cache}"
	if { [[ "${deferral_restrictions_checksum_cache}" == "0" ]] && [[ ! -f "${APLICATION_ACCESS_MANAGED_PLIST}.plist" ]]; } || [[ "${deferral_restrictions_checksum_cache}" == "$(md5 -q "${APLICATION_ACCESS_MANAGED_PLIST}.plist" 2>/dev/null)" ]]; then
		...
		[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_cache is: ${deferral_restrictions_cache}"
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions is: ${deferral_restrictions}"
...
check_deferral_restrictions() {
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_response is:\n${deferral_restrictions_response}"
...
workflow_check_software_status() {
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ ${#mdmclient_macos_installers_deferred_array[@]} -gt 0 ]] || [[ ${#mdmclient_macos_msu_deferred_array[@]} -gt 0 ]] || [[ ${#mdmclient_non_system_deferred_array[@]} -gt 0 ]]; }; then
		...
		if [[ ${#mdmclient_non_system_deferred_array[@]} -gt 0 ]]; then
			...
			{ [[ "${deferral_restrictions}" == "FALSE" ]] && [[ "${verbose_mode_option}" == "TRUE" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_non_system_array is:\n${mdmclient_non_system_array[*]}"
		fi
	fi
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ ${#mdmclient_macos_msu_array[@]} -gt 0 ]]; then
		...
		for mdmclient_macos_msu_item in "${mdmclient_macos_msu_array[@]}"; do
			[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_item is: ${mdmclient_macos_msu_item}"
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ ${#mdmclient_macos_installers_array[@]} -gt 0 ]] || [[ ${#mdmclient_macos_msu_major_upgrade_array[@]} -gt 0 ]]; }; then
		...
		if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ -n "${install_macos_major_upgrades_target}" ]] && [[ ${#mdmclient_macos_installers_array[@]} -gt 0 ]]; then
			...
			for mdmclient_macos_installer_item in "${mdmclient_macos_installers_array[@]}"; do
				[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installer_item is: ${mdmclient_macos_installer_item}"
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ "${mdmclient_macos_installer_target}" != "FALSE" ]] && [[ "${macos_installers_list}" != "FALSE" ]]; then
		...
		for macos_installer_item in "${macos_installers_array[@]}"; do
			[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item is: ${macos_installer_item}"
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_major_upgrade_target}" != "FALSE" ]] && [[ ${#macos_msu_array[@]} -gt 0 ]]; then
		...
		for macos_msu_item in "${macos_msu_array[@]}"; do
			[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_item is: ${macos_msu_item}"
...
install_macos_mdm() {
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_workflow_type is: ${install_macos_mdm_workflow_type}"
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_version is: ${install_macos_mdm_version}"
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_api_error is: ${install_macos_mdm_api_error}"
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_start_error is: ${install_macos_mdm_start_error}"
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_start_error is: ${install_macos_mdm_start_error}"
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_start_timeout is: ${install_macos_mdm_start_timeout}"
	...
	if [[ "${auth_error_jamf}" == "FALSE" ]] && [[ "${install_macos_mdm_api_error}" == "FALSE" ]] && [[ "${install_macos_mdm_start_error}" == "FALSE" ]] && [[ "${install_macos_mdm_start_timeout}" == "FALSE" ]]; then
		...
		[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_workflow_type is: ${install_macos_mdm_workflow_type}"
		[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_timeout_seconds is: ${install_macos_mdm_timeout_seconds}"
		if [[ "${install_macos_mdm_workflow_type}" == "UPGRADE" ]] || [[ "${install_macos_mdm_workflow_type}" == "UPDATE" ]]; then
			while read -t "${install_macos_mdm_timeout_seconds}" -r log_line; do
				...
				if [[ $(echo "${log_line}" | grep -c 'phase:PREFLIGHT') -gt 0 ]]; then
					...
					[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_complete_percent is: ${install_macos_mdm_complete_percent}"
	...
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_timeout_error is: ${install_macos_mdm_timeout_error}"
	[[ "${verbose_mode_option}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_mdm_phase is: ${install_macos_mdm_phase}"
...
set_display_strings_language() {
    ...
    ### Woz Custom Display Variables ###
			...
			# UPDATED Variables for SUPERMAN 5b3 CONFIRMED | 9/18/24
			# *** Use the line below for NOTIFICATION LINE(S)! ***:
			# $dialogUpdates\n$otherUpdates\n\nRestart Required? : **$restartRequired**

			# schema setting to enable displaying of additional update information | 8/8/24 | *** NEED TO IMPLEMENT ***
			...
			if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_msu_minor_update_target}" != "FALSE" ]]; then
				...
				macOSVersionMajor=$(/usr/bin/sw_vers | grep "ProductVersion" | awk '{print $2}' | sed 's/\..*//')
					...
					# curl the latest SOFA release for total CVEs (of latest release | need to script matching of other versions to SOFA information)
					...
					# if CVE count is less than 5, let's remove that line from the display dialog to possibly 'encourage' updating without showing a low number LOL
...
dialog_user_schedule() {
	...
	display_accessory_datepicker_start_epoch=$((workflow_time_epoch + (deferral_timer_minutes * 60) + 1))
	...
	0)
		...
		if [[ -n "${schedule_workflow_active_option}" ]]; then
			...
			if [[ -n "${workflow_scheduled_install_adjusted}" ]]; then
				workflow_scheduled_install="${workflow_scheduled_install_adjusted}"
				[[ "${scheduled_install_user_choice_adjusted}" == "TRUE" ]] && log_super "Status: Setting new user selected scheduled installation of $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} which is the start of a workflow active time frame that closest matches the user's choice."
			else
				log_super "Warning: The current user selected scheduled installation of $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} is overriding the schedule workflow active option because no coordinating time frame could be resolved."
			fi
		fi
		defaults write "${SUPER_LOCAL_PLIST}" WorkflowScheduledInstall -string "${workflow_scheduled_install}"
		workflow_scheduled_install_epoch=$(date -j -f %Y-%m-%d:%H:%M:%S "${workflow_scheduled_install}:00" +%s)
...
dialog_user_auth() {
	...
	ibm_notifier_array=(-type popup -bar_title "${display_string_user_auth_title}" -icon_path "${display_icon}" -icon_width "${display_icon_size}" -icon_height "${display_icon_size}" -main_button_label "${display_string_restart_now_button}")
